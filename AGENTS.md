...
`{{PROJECT_NAME}}`, `{{ID_PREFIX}}`, and `{{DATE}}` inside `templates/**/*.tmpl` are literal tokens substituted by `scripts/init_scribe_coding_project.sh`. Never replace them with real values inside `templates/`, and never strip a file's `.tmpl` suffix by hand. A new template file should mirror the target project's relative path exactly and end in `.tmpl`; the init script picks up any `*.tmpl` automatically, so no separate registration is needed.
